...
gulp.task('copyTinymce', function(done) {
  if (fs.existsSync('../tinymce/js/tinymce/tinymce.min.js')) {
    ...
    const html = fs.readFileSync('./build/index.php', 'utf8');
    fs.writeFileSync('./build/index.php', html.replace('/tinymce/tinymce.min.js', url));
    done();
  }
});

// Generate list of available skins and content css:es to populate select field in index.php
...
gulp.task('buildSkinSwitcher', (done) => {
  ...
  const html = fs.readFileSync('./build/index.php', 'utf8');
  fs.writeFileSync('./build/index.php', html.replace('/** ADD_DATA */', data));
